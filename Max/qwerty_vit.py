...
def generate_compensation_model(q_model, train_loader, args):
    ...
    for block_id in range(len(q_model.blocks)):
        ...
        # 把前一个block补偿之后的输出作为 当前block计算W和b的输入，第0个block则将forward_before_blocks(即self.blocks)前的特征作为输入
...
parser.add_argument("--model", default="deit_small_distilled", choices=['vit_small', 'vit_base', 'deit_tiny', 'deit_small', 'deit_base', 'swin_tiny', 'swin_small',  'deit_tiny_distilled', 'deit_small_distilled', 'deit_base_distilled'], help="model")
