...
def create_transform(aug_type=None, mean=None, std=None, crop_pct=None):
...
def compute_amax_format(model, calib_method=None, local_rank=0, log_file=None):
        ...
        if isinstance(module, quant_nn.TensorQuantizer):
            if module._calibrator is not None:
                if isinstance(module._calibrator, calib.MaxCalibrator):
                    ...
                    if local_rank == 0:
                        write('finished calib of {} using max'.format(name), log_file=log_file)
                else:
                    module.load_calib_amax(calib_method)
                    if local_rank == 0:
                        write('finished calib of {} using {}'.format(name, calib_method), log_file=log_file)

def collect_stats_format(model, calib_data, local_rank=0, log_file=None):
    for name, module in model.named_modules():
        if isinstance(module, quant_nn.TensorQuantizer):
            if module._calibrator is not None:
    ...
    write('local_rank : {} calib_data shape : {} value : {}'.format(local_rank, calib_data.size(), calib_data[0, 0, 0, :5]), log_file=log_file)
                ...
                for k in _module_._parameters:
                    if _module_._parameters[k] is not None:
                        if (k == 'weight') and hasattr(_module_, 'weight_quantizer'):
                            n_bits_ = _module_.weight_quantizer.num_bits
